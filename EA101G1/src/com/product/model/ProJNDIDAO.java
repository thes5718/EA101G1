...
public class ProJNDIDAO implements ProDAO_interface {
	...
	private static final String INSERT_STMT = "INSERT INTO PRODUCT(P_ID,PT_ID,P_NAME,P_PRICE,P_INFO,P_IMAGE,P_SALES,P_STOCK,P_STAT) VALUES ('P'||LPAD(TO_CHAR(SEQ_P_ID.NEXTVAL),3,'0'), ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	private static final String UPDATE = "UPDATE PRODUCT SET  PT_ID=?, P_NAME=?, P_PRICE=?, P_INFO=?, P_SALES=?, P_STOCK=?, P_SALES=?, P_ADD_DATE=?, P_STAT=? WHERE P_ID=?";
	...
	public void update(ProVO proVO) {
		...
		try {
			...
			pstmt.setString(5, proVO.getP_info());
			pstmt.setInt(6, proVO.getP_sales());
			pstmt.setDate(7, proVO.getP_add_date());
			pstmt.setInt(8, proVO.getP_stock());
			pstmt.setInt(9, proVO.getP_stat());
			pstmt.setString(10, proVO.getP_id());
